<!-- Generado por scripts/gen-module-graph.ts — no lo edites a mano.
     Ejecuta `pnpm run gen-module-graph` para regenerarlo. -->

# Grafo de dependencias de módulos

[English](module-graph.md) | Español

Dependencias entre los paquetes del agent harness `@deepseek-ai/dsh-*`, derivadas de los `peerDependencies` de cada paquete (la señal canónica de dependencia en tiempo de ejecución) y agrupadas según la jerarquía `packages/<group>/<pkg>`. Una arista `a --> b` significa que el paquete `a` depende del paquete `b`. Los nombres llevan eliminado el prefijo `@deepseek-ai/dsh-`.
